...
| 峰值 `phys_footprint` | `1126.4 MiB` | **`569.0 MiB`** |
| 峰值 `ps rss` | `1058.9 MiB` | `581.2 MiB` |
| 留存堆 | `257.8 MiB` | `256.7 MiB` |
| 可见终态 | `15.2s` | **`14.259s`** |
| 目录覆盖率 | `100%` | `100%` |

**报告内存必须写明口径。** `ps rss`、`phys_footprint`、`runtime/metrics` 活跃堆在本场景下分别是
`581.2 / 569.0 / 256.7 MiB`，不写口径的内存数字没有意义（R-057 §4c.5）。

**ADR-0056 门禁 2 的峰值部分仍未达成**，但原因已经不是 churn：留存堆 `256.7 MiB`，而 `GOGC=100`
的定义就是堆涨到活跃量的 `2×` 才回收，门禁写的"峰值/稳态 `<=1.2`"由构造决定就达不到。
这是门禁本身的规格问题，待决策者裁决（ADR-0057 后续工作第 1 条）。
