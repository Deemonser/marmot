...
**已实施**：证据装配（§14.2）、规则层、建议面板与发送内容预览。
**未实施**：`Advisor` 端口与适配器（§14.5）、脱敏代码路径（§14.3）、出网（§14.4）。
当前实现全程不出网，全部结果来自本机规则目录。

模型端行为完全未验证（R-062 §5），本节只约束"送什么进去、收回来怎么处理"，
不承诺任何准确率。
...
- 首次触发弹一次授权说明，此后不再弹；
- API key 由用户自带，存 macOS Keychain，不落配置文件、快照、日志或崩溃报告。
