...
### 13c 两种执行方式，以及守卫的两种身份

移入废纸篓**不释放空间**——废纸篓在同一个卷上，移动只是改名。实测：删除约 33 GB 之后
可用空间一字未变。所以清理计划有两种执行方式（ADR-0063）：

| | 移入废纸篓（默认） | 直接删除 |
| :-- | :-- | :-- |
| 释放空间 | 否，要清空废纸篓 | **是，立即** |
| 可撤销 | 是 | **否** |
| 不可恢复守卫 | **建议**（用于更正模型主张） | **硬拒绝** |

`Permanent` 随每次计划传递，**不落盘、不记忆、执行后复位**：不可逆的动作应当每次被选择。
界面上是收集区里的复选框而不是第二个按钮——并排两个破坏性按钮就是按错那个的标准做法。

守卫的两种身份是这条决策成立的前提：废纸篓可逆，所以 `IrreplaceableReason` 在那条路径上
只是更正；直接删除不可逆，同一层判断就必须是拒绝。**凡是删了无法重建的，只能进废纸篓。**

`platform.RemovePermanently` 不判断"什么能删"，只拒绝空路径与 `/`；
该判断是应用层策略。逐条校验（`matchesSnapshotNode` / `validateCleanupItem`）两条路径共用，
一字未改。
