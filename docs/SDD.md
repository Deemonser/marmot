...
面板在列表上方常驻一行，说明自动放入了几项、以及**刻意留下了几项等你确认**；
留下的部分另有一个"加入其余 N 项"批量按钮——它是显式动作、作用在已经在屏幕上的列表上，
这是它与"自动填满"的区别。
...
- **第二次不重复自动放入。** 规则项已经在收集区里，重放会把用户在等待期间**移除过的项
  又放回去**。
- **两次等待分开计时、分开展示。** `advisorBusy` 独立于 `adviceBusy`；面板在列表上方
...
结果页左下角是 Collector（ADR-0018），右下角空置，AI 分析入口放右下角。
建议面板中每条建议可直接拖入左下 Collector，与空间图弧、目录列表行走同一条收集路径。
