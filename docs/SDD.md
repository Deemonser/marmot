...
### 13e 删除后自动重新扫描，而不是回写快照

ADR-0009 禁止把清理结果回写为扫描事实，这条是对的：把删掉的节点减掉，会得到一张
**和任何东西都不一致**的图——尤其和一块同时被其他进程写着的磁盘。

但"提示用户自己去重新扫描"把接续的责任推给了用户。现在删除成功后**自动触发一次重新扫描**，
既守住 ADR-0009（重扫，不是打补丁），又让视图自己接上。全盘实测约 9.5 秒。

**实时更新（FSEvents 监听）不做**，理由记在这里以免被反复提出：全盘监听在一台正常使用的
机器上是持续不断的事件流（浏览器缓存、构建产物、日志每秒都在变），空间图会永远在动，
而这张图的价值恰恰来自它是**某一时刻的一致快照**——`residue` 分区、下限、干扰度分级全都
建立在"同一时刻"之上。一张永远在抖的图读不出任何结论。
