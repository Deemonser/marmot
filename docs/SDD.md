...
- Platform 实现文件系统、权限、删除、卷、Quick Look 和 Finder 能力。
...
角度，低于它的兄弟折叠成一个近黑的中性块（`#3b3b3f`：原版当前版本实测 `#35363B` 压在 `#25272D` 页面上，
我们保持同样的提亮幅度而不是照抄色值；R-055 记录的 `#888787` 来自旧版），连这一块都不够宽则整组不画、背景透出——
...
分母取快照在 `CreateSnapshot` 时记录的值；有上一次完成扫描的历史时，分母换成**上次的最终字节**，且若上次也记了
节点终值，进度取**字节分数与节点分数各自封 1 后的平均**（ADR-0053 第二次修订：遍历是广度优先，队尾是文件多、
字节少的深层目录，字节分数在那一段停在封顶而节点分数仍在爬）。历史文件 `scan-totals.json` 每根一条
`{"bytes","nodes"}`，旧的裸数字格式按"只有字节"读取。扫描中每 2s 写一行进度日志（用时、阶段、计数、两个口径的百分比）。辅助卷必须**前置计入**（扫描前已从 `statfs` 得知，且必然进入
...
`展开 "名"`（目录且有 `enter`）、`在 Finder 中显示`、`在终端中打开`（原版还有 `预览`，2026-09-09 使用方判定
无用，已去掉——空格预览仍在，ADR-0069 修订记录）、
...
smoke test 和真实只读全盘样本完成前，不宣称达到发布级全盘目标。
...
发布前仍必须完成真实签名/TCC、真实 Wails 窗口中的 Quick Look/Finder smoke test
和只读全盘样本性能验证；这些验证不能由本机 ad-hoc 构建替代。
...
- **进入任一层级时没有焦点行**（原版如此；此前首行默认取焦点，读起来像一个永远选中的项）。键盘焦点由
  方向键、PageUp/Down、Home/End 或 Tab 产生，没有焦点时第一次按键落在首行（向上则末行）；最后一次按键
  `3s` 后焦点高亮与焦点环一起自动消失。悬停高亮与单击选中不受影响——选中是一个决定，不是光标；
...
参考产品允许永久删除；Marmot 自 ADR-0063 起同样直接删除，差异在于三次确认与 `cleanup.DeleteBlock`
硬边界，且执行前必须重新校验文件身份和计划版本。R-009 的 P0 清单是首个体验垂直切片的验收入口，
...
5. **位置在收集区的对角**：结果页上收集区在右下（ADR-0073），提示条因此在左下、"分析优化"按钮之上
   （`left: 18px; bottom: 62px`），不会落在它多半正在谈的面板或徽标上；源页仍在右下。
...
不是独立文件；按站点清理意味着写入应用的数据库，这远超"删除独立文件对象"的边界
（§10/§13c：清理只删除文件系统对象，不写入任何应用的数据库），且浏览器持有该文件时写入有损坏风险。
...
**容量条按占用率连续变色。** 此前是硬编码的 `#e0492a`（警示红），无论多满都一样——
**一个无论测什么都读同一个值的仪表是装饰，不是信息**。第二版做成三档（绿/琥珀/红），但 84% 与 85%
是两种颜色、84% 又和 70% 一样，原版不是这样：原版的颜色是占用率的连续函数，84% 是橙色。现在按占用率
在 HSL 上分段插值：`≤50%` 保持绿，此后经黄绿、黄、琥珀（70%）、橙（85%）到红（100%）连续变暖；
在 HSL 而不是 RGB 上插值，是为了绿到黄之间不经过脏橄榄色。锚点仍按"读到这个数该不该行动"选：70%
值得计划一次清理，85% 以上开始出故障（构建、更新、Time Machine 快照）。**条下的剩余空间数字取与条
相同的颜色**——绿条配红字是对同一块盘的两种意见。指针悬停计量区时，这一行左侧淡入"已使用 X"，
移开即隐（原版如此）。判据在 `frontend/src/meter.ts`，含"容量读不出来时读作空、而不是读作满"
——为一个读不到容量的卷亮红灯是无中生有。**副标题只有容量与角色**（如"494.4 GB 启动盘"）：成员卷数
（"2 个技术卷"）和 APFS 卷组说明的悬停提示都去掉了——那是存储源如何装配的事实，不是关于磁盘的事实，
选盘的人用不到；成员事实仍留在 `StorageSource` 读模型里，只是不进版面。
...
**一个会直接删掉几十 GB 的工具，如果不留下自己做过什么的记录，出事的人就没法排查。**
...
- 提示按逐条结果组织："已处理 N 项，M 项未执行：<名称>（原因）…"；
...
1. **待确认**——`advice.items` 中不在收集区的项，含 `manual` 项。每行两行高：第一行风险色点、名字，
   **标签紧跟名字**（来源：规则名或 `AI · 置信度`；恢复代价；风险；风险之后是**判定理由**标签——
   ...
   > **位置与几何已由 ADR-0073 取代（2026-09-09）**：面板在**列表列下方**、右缘贴 16px 页边距，展开时盖住列表列
   > 而不是空间图列，列表列同时由 38% 放宽到 46%（240ms 过渡），轮盘直径不变、完整可见；面板在所有状态取同一宽
   > `(100vw − 32px) × .46`，徽标骑面板**右**下角、在窗口角上（动作条整体镜像：删除、文字、徽标；单位进徽标第二行，R-075 §4）；已收集列表**没有
   > 自己的 26vh 上限**，改为面板能给的全部高度 `calc(100vh − 106px)`——上限必须写在列表上，网格行 `1fr → 0fr` 的折叠
> 只在容器按内容定高时有插值，被面板夹成定高就成一跳（ADR-0073 §3）；面板顶到面包屑之下，列表在其内滚动；
> "分析优化"按钮换到左下角。
   > 另注：本节的"待确认"区自 `31c001c`（勾选式收集区：行带勾选框，只有勾上的进计划）起已不存在，下文关于两区的
   > 描述是历史记录，待整体修订。
   ~~展开时面板盖满左侧空间图列，待确认列表拿到全部剩余高度，已收集列表限高 `26vh`。~~
   ~~面板在所有状态下只有一套几何：左边距 33px、底边距 24px、宽为地图列的 62% 减 17px~~；徽标骑在面板左下角
   ——圆心在面板左缘内 3px、底边上方 17px，50px 的圆盘因此向左悬出 22px、向下悬出 8px（ADR-0066 第二次修订，
   按原版截图 2x 逐像素量得）。折叠只改高度：待确认区 `height` 从 `calc(100vh - 54px)` 过渡到 0；已收集区是一个
   网格行从 `1fr` 过渡到 `0fr`——从内容的真实高度开始收，第一帧就在动（此前用 `max-height: 26vh` 起步，几行
   内容时前大半段什么都看不到）；行内文字随之淡出，面板背景等文字淡完再淡去，任何时候都没有文字站在裸页面上。
   使用方反馈第一版折叠时"宽度和边距在跳动"，原因正是两种状态各有一套左/下/宽。同一原则用到空态：
   **空态就是同一条动作条放在一个不上色的面板里**，投放环和说明文字的位置与第一次投放之后完全相同。
   **面板只剩动作条时**（列表折叠，或空态且有拖拽在途），动作条后面是一条 36px 高、两端全圆的胶囊，
   从徽标圆心底下伸出，被徽标外圈的页面色切口切开左端——它是徽标背后的底板，不是旁边的独立药丸。
   **徽标是一个 50px 元素**，环由边框和四层 box-shadow 画出，三态之间靠过渡互相变形（原版三张截图实测）：
   空闲是蓝灰色的外环、细中环和 26px 圆盘；拖拽在途时外环变蓝、中环加粗、圆盘缩成 14px 蓝点，并向外反复扩散
   一圈淡出的蓝环（声呐）；有内容时是蓝环、3px 页面色空隙和 40px 蓝灰圆盘，数字弹出。拖到面板上不放大、
   面板不描边；拖入受保护对象时环回到灰色并画红色斜杠，拖拽提示的色点变红。
   **拖拽提示**是色点加白字：色点的圆心在指针热点上，名称一行、大小一行挂在右侧，没有色块与阴影；
   拖拽全程指针保持箭头。
   **动作条上的圆环徽标是按钮**：有内容时点击它折叠或展开面板主体（两个区一起，`dockOpen`），只剩动作条。
   **默认折叠，拖入不展开**：原版对一次投放的回答是徽标里的数字和"GB 已收集"，不是列表。开始新分析和
   "全部加入"会展开——那两种情况放进去的是用户没有逐个看过的东西；收集区被清空（逐个移除或删除执行完）
   时回到折叠，待确认区正在显示时除外。删除进行中圆环是进度指示，点击无效。已收集行的移除按钮悬停时
   只把色点换成叉，不加底色、不换手形光标——面板、行、按钮三层底色叠在一起太多了。
...
若在更深的环上，按节点 id 让投影弧呼吸（`Sunburst.breathingNodeId`）。已收集的弧呼吸时 `fill-opacity` 在 `1 ↔ .2`
之间全幅摆动（`slice-breathe-staged`）：此前是把 fill 抬到 `.6` 再套普通弧的 `opacity 1 ↔ .45`，得到 `.6 ↔ .27`，在半透明
里看不出来（2026-09-09 使用方指出）；全幅摆动让弧在"在这里"和"已排队"两种样子之间交替，隔着轮盘也读得出。
已收集的弧**仍响应指针**：悬停呼吸并联动列表，右键菜单给出"移出收集站"；但不可点击进入、不可再拖（对象已排队
删除）。此前 `.is-collected` 带 `pointer-events: none; animation: none`，前者让弧本身悬停无反应，后者以同等
特异性覆盖了 `.is-breathing` 的动画，使收集区行悬停时弧只是变亮而不呼吸——与 ADR-0066 §"悬停联动"的意图相反
（2026-09-09 使用方指出）。拖动期间整个轮盘的命中测试由 `.app-shell.is-dragging` 关掉，与此无关。
联动是**双向**的（2026-09-10 补齐反向）：悬停已收集的弧时，收集区里对应的行以同一节奏（0.9s）呼吸背景并
`scrollIntoView` 到视线中（面板展开时）。此前有一条守卫把落在"被拉走的弧"上的 `hoveredArcKey` 一律清空，那是弧
还不命中测试时的正确假设，弧恢复命中后它就成了"已收集的弧悬停不呼吸"的原因；守卫收窄为只清正在拖动的那一个。
