...
状态：已完成，结论由 ADR-0028 接受；格式 POC、生产 `SnapshotStore` 适配器和 Darwin 原生扫描主循环已接入，真实二进制快照终态 smoke 已完成，但 15 秒性能门槛仍未达成
...
本机 macOS arm64、Go 1.25.13、原生主循环接入前的 Go 扫描路径只读 smoke（历史对照）：
...
格式 POC、生产 Store 适配和 Darwin 原生扫描接线已完成，验证结果如下；这些结果确认了真实终态链路，
但不替代 15 秒发布性能门槛：
...
- 当前查询使用 Go `os.File.ReadAt`（Unix 上对应有界 pread 语义），生产 Store 已接入；mmap 映射尚未实现。
- 原生扫描已通过取消、超过 bulk 输出上限、普通高扇出、原生队列满载和批量 emitter 回归测试；
  Darwin 原生回调以 `[]scan.Node` 批量进入 Application，旧单节点 `Scan` 接口仍由适配层兼容。
  队列满载时由当前 worker 处理本地溢出任务，避免所有 worker 同时等待有界队列；目录批缓冲使用
  `malloc`，避免每个目录清零约 9 MiB 的大批缓冲。
- 最新本机真实 `/` 纯扫描三次为 `13.99s`、`21.41s`、`12.92s`，节点数约 273.4 万，中位数约
  `13.99s`；三次完整二进制快照终态为 `20.47s`、`23.26s`、`20.87s`，中位数约 `20.87s`，
  `FinishSnapshot` 中位数约 `4.24s`。三次完整终态均为 `completed_with_issues`，首层和根节点查询
  可用；文件系统内容在样本间有变化，15 秒终态目标仍未达到。
- Writer 发布前会校验完整数据帧、索引和计数；已提交快照通过 `OpenCommitted` 打开，首个查询不再
  重复扫描整个数据文件。固定宽度临时记录和索引记录在热点循环中复用编码缓冲。

剩余验证与性能工作：

1. 在同一文件系统状态下对比节点数、问题数、三种大小、分页、Map 投影、Preview/Reveal 身份读取和取消结果；
2. 记录 cgo 批量调用次数、峰值 RSS、快照文件大小和重启恢复时间；
3. 以相同根路径、权限、节点规模和冷/热缓存对比旧 SQLite 基线；
4. 针对约 4.24 秒的 `FinishSnapshot` 和纯扫描波动继续 profile，确认是否存在稳定的可优化热点。
...
- 当前 POC 和 Store 适配只验证合成快照及小型应用样本，真实 `/` 已验证原生纯扫描稳定性，但完整快照终态和
  15 秒目标仍是未完成门槛；
...
- 下一步针对真实终态约 `20.87s` 中的扫描波动和约 `4.24s` 的 `FinishSnapshot` 做 profile 与分段优化，补齐 cgo/RSS/文件大小/恢复证据，不修改 Wails 公共接口、清理领域不变量或前端 DTO；
