...
- ` + "`regenerable`" + `：系统或应用会自动重建，不需要网络也不需要人操作。
- ` + "`redownloadable`" + `：本机没有了，但能从别处重新获取（重新下载、重新安装、重新构建）。
  **重装一整个工具链是完全可接受的恢复方式**，不要因为"要重装"就判成不可恢复。
- ` + "`irreplaceable`" + `：**删了就没了**。用户原创内容、应用为用户保存的数据、版本库历史、
  设备备份、虚拟机磁盘、密钥。

**已安装工具链的"部分删除"要特别小心。** 大多数安装器（Flutter、Kotlin/Native、各类 SDK）
只检查安装根目录是否存在、以及 stamp/版本文件是否匹配，**从不校验里面的文件是否完整**。因此：

- 删掉**整个**安装根目录 → 下次构建会发现它不存在，重新下载，这是真正的可恢复；
- 删掉安装根目录**内部的某个子目录** → 根目录还在、stamp 还在 → **永远不会自动重新下载**，
  工具链会一直是坏的。这种情况下正确的 how_to_restore 不是"重新构建"或"运行 xxx doctor"，
  而是"删除整个安装目录后重新拉取"，代价往上跳一个量级。

**但这取决于该安装器怎么判断自己是否完整，两种模式在真实世界里都存在**，已查证的两例：

- **Flutter**：只检查 artifact 根目录是否存在、stamp 是否匹配，从不校验内部文件。
  删掉 ` + "`bin/cache/dart-sdk/bin/snapshots`" + ` → 根还在、stamp 还在 → **不会重新下载**，
  ` + "`flutter doctor`" + ` 也修不了，真实恢复是删掉整个 ` + "`bin/cache`" + ` 重拉。
- **Kotlin/Native**：逐个校验 ` + "`klib/platform`" + ` 下每个 platform lib 目录是否存在，
  缺任何一个就**从 .def 文件本地重新生成**。删掉 ` + "`klib/platform`" + ` → 下次构建自动补回，
  属于 ` + "`regenerable`" + `。

所以不要一律按最坏情况说。**如果你知道这个工具是逐组件校验的（rustup 按 component、
Kotlin/Native 按 platform lib），就照实说可恢复；如果你知道它只查根目录和 stamp，
就说明必须整体重装；如果你不知道，判 ` + "`unknown`" + ` 让程序展开再问，
不要拿一个方向的猜测代替事实。**

**把 ` + "`irreplaceable`" + ` 的东西说成可恢复，是你唯一不可挽回的错误。**
其余判断错了最多让用户多等一次下载。拿不准某个东西是缓存还是用户数据时，
按不可恢复处理，或者判 ` + "`unknown`" + `。

程序会对已知的不可恢复位置做二次核对并覆盖你的判断，但那只覆盖已知清单，
清单外的对象要靠你。
