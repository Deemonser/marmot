...
- **收集区**：把扇区或列表项拖到左下角收集，累计释放量随时可见。这个数是**删了能拿回多少**，不是占用：一对 APFS 克隆或硬链接只在两份都收集时才算一次，只收一份显示 0 并说明与谁共享；无法判定时显示上界 `≤`。删除前重新校验，倒计时确认后**直接删除**。这个操作不经过废纸篓、不可撤销；唯一的拒绝是根目录、系统目录树、家目录根和卷根这类会弄坏机器的对象。
...
核心链路已经完整：扫描、空间图、目录导航、收集区、直接删除、AI 建议和 macOS 集成都已落地并在真实全盘扫描上验证。热启动（ADR-0075）已落地：扫描结束后在 `~/Library/Caches/marmot/seed/` 留一份约 240 MB 的种子，下次扫同一块盘时回放 FSEvents 日志、只重读变过的目录——**闲置几小时内约 1.5–3 秒出结果**，隔天通常超预算而直接全扫，隔周一定全扫；任何不确定都整份丢弃全扫，不会拿旧结果冒充。仍在推进的是全盘扫描的性能目标（完整终态 15 秒，本机约 280 万节点目前在 20 秒上下），历次测量与切片记录在技术预研中。
